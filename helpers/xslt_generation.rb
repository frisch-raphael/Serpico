  ...
  while i < metacharacters.length
    ...
    when 'ツ'
      ...
      while j + i < metacharacters.length
        if (i + j == metacharacters.length - 1) || (metacharacters[j + i][1] == 'ツ')
          break
        elsif (i + j == metacharacters.length - 1) || (metacharacters[j + i][1] != 'π')
          tree_valid = false
          break
        end
             j += 1
...
def generate_xslt(docx)
  ...
  raise TemplateVerificationError.new(error, tree) unless tree_valid
...
###############################
  ...
  begin
    ...
    error = e.message
    raise ReportingError, "This exception was rescued while verificating the main XSLT well-formedness: <br/><br/>#{CGI.escapeHTML(error)}"
  end
